...
const ALL_EVENT_TYPES: EventTypeConfig[] = [
  ...
  { key: 'POKE',      icon: '👉', labelKey: 'poke.sendTitle',    color: '#34B7F1', bgColor: '#162D3D' },
  { key: 'CUSTOM',    icon: '✏️', labelKey: 'events.custom',    color: '#4ECDC4', bgColor: '#0F2E2C' },
];

/** These are the types available for manual selection in the UI */
export const EVENT_TYPES: EventTypeConfig[] = ALL_EVENT_TYPES.filter(t => t.key !== 'POKE');

/** This map contains ALL types, including system types like POKE, for rendering purposes */
export const EVENT_TYPE_MAP = Object.fromEntries(
  ALL_EVENT_TYPES.map((t) => [t.key, t])
