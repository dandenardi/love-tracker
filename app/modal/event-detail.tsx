...
export default function EventDetailModal() {
  ...
  const [moodTag, setMoodTag] = useState<string | null>(event?.mood_tag || null);
  const [occurredAt, setOccurredAt] = useState(new Date(event?.occurred_at || Date.now()));
    ...
    return (
      <View style={[styles.container, { backgroundColor: c.background, justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <TouchableOpacity onPress={() => router.back()}>
          <Text style={{ color: c.primary, marginTop: 16 }}>{t('common.back')}</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: c.background }]}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={[styles.header, { borderBottomColor: c.border }]}>
          <TouchableOpacity onPress={() => router.back()}>
            <Text style={[styles.headerBtn, { color: c.textSecondary }]}>{t('common.close')}</Text>
          </TouchableOpacity>
          <Text style={[styles.headerTitle, { color: c.text }]}>{t('events.editEvent')}</Text>
          <TouchableOpacity onPress={handleSave}>
            <Text style={[styles.headerBtn, { color: c.primary, fontWeight: '700' }]}>{t('common.done')}</Text>
          </TouchableOpacity>
        </View>

        <ScrollView contentContainerStyle={styles.scroll}>
          {/* Event Header Info */}
          <View style={[styles.infoCard, { backgroundColor: cfg?.bgColor || c.surface, borderColor: cfg?.color || c.border }]}>
            <Text style={{ fontSize: 48 }}>{cfg?.icon || '📝'}</Text>
            <Text style={[styles.infoType, { color: cfg?.color || c.text }]}>{t(cfg?.labelKey || 'events.custom')}</Text>
            <Text style={[styles.infoLogged, { color: c.textMuted }]}>{t('events.loggedAt')}: {format(new Date(event.logged_at), 'PPpp')}</Text>
            {event.is_private === 1 && (
              <View style={styles.privateBadge}>
                <Text style={styles.privateBadgeText}>🔒 {t('events.privateBadge')}</Text>
              </View>
            )}
          </View>
            ...
            {[0, 1, 2, 3, 4, 5].map((i) => (
              <TouchableOpacity
                ...
                style={[
                  styles.intensityBtn,
                  {
                    backgroundColor: i <= intensity && i > 0 ? ((cfg?.color ?? c.primary) + '40') : c.surface,
                    borderColor: i <= intensity && i > 0 ? cfg?.color ?? c.primary : c.border,
                  },
                ]}
              >
            ...
            {MOOD_TAGS.map((m) => (
              <TouchableOpacity
                ...
                style={[
                  styles.moodBtn,
                  {
                    backgroundColor: moodTag === m.emoji ? c.primary + '30' : c.surface,
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 11, fontWeight: '700', letterSpacing: 1.2, marginBottom: 10, marginTop: 24 },
  ...
  privateBadge: {
    ...
    borderRadius: 20, backgroundColor: '#7C3AED22', borderWidth: 1, borderColor: '#A78BFA',
  },
